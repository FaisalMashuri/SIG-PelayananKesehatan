...
const editPelayanan = (id, kelurahan_id, pelayanan_id, nama, ket) => {
  return new Promise((resolve) => {
    const sql = `UPDATE distribusi_pelayanan distribusi_pelayanan SET kelurahan_id=${kelurahan_id},pelayanan_id=${pelayanan_id},sarana_pelayanan="${nama}",keterangan="${ket}" WHERE distribusi_pelayanan_id = ${id}`;
    db.query(sql, (err, res) => {
      if (err) console.log(err);
      resolve("Success");
    });
